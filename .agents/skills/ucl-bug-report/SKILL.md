---
trigger: { on_intent: ["回報 bug", "報 bug", "bug report", "撞到 bug", "回報錯誤", "系統異常", "這裡壞了", "文件過時", "說明跟現況不符", "文件寫錯", "這份文件沒更新", "照文件做結果不一樣", "提示缺參數", "錯誤訊息指錯地方", "容易踩的坑", "這裡很容易做錯", "第一次一定會卡", "流程可以簡化", "這步驟是多的", "可以少一步", "流程建議", "改善建議"] }
name: ucl-bug-report
description: |
